...
topicToViz <- grep('npd', top.words)[1] # Or select a topic by a term contained in its name
# select to 40 most probable terms from the topic by sorting the term-topic-probability vector in decreasing order
top40terms <- sort(fit$terms[topicToViz,], decreasing=TRUE)[1:10]
words <- names(top40terms)
# extract the probabilites of each of the 40 terms
probabilities <- sort(fit$terms[topicToViz,], decreasing=TRUE)[1:10]
